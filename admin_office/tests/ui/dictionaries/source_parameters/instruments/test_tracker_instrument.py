...
@pytest.fixture()
def clean_up_tracker_data(
        authorization_in_admin_office_with_token,
):
    """ Удаляет данные после выполнения всех тестов на вкладке 'Трекер' """
    tracker_id = get_source_id_by_naming(data_instruments_card['naming'])
    if tracker_id:
        delete_source_by_id(tracker_id)
...
class TestTracker:
    @staticmethod
    @pytest.mark.smoke
    @allure.title("Создание записи в справочнике 'Инструменты'. Вкладка 'Трекер'")
    ...
    def test_create_tracker(
        ...
        admin_instruments_page.instruments_card.fill_tools_type_dropdown('Трекер')
        ...
        admin_instruments_page.instruments.page.reload()
    ...
    @staticmethod
    @pytest.mark.smoke
    @allure.title("Редактирование записи в справочнике 'Инструменты'. Вкладка 'Трекер'")
    @allure.story(jira.JIRA_LINK + 'MDP-1166')
    ...
    def test_edit_tracker(
        ...
        created_tracker_id = str(get_source_id_by_naming(data_instruments_card['naming']))
        admin_instruments_page.instruments.page.get_by_role('row', name=created_tracker_id).get_by_role(
            'link').click()
        data_instruments_card['name'] = data_instruments_card['name'] + 'ed'
        data_instruments_card['url'] = 'https://ya.ru'
        admin_instruments_page.instruments_card.fill_tools_name_field(data_instruments_card['name'])
        admin_instruments_page.instruments_card.fill_tools_url_field(data_instruments_card['url'])
        admin_instruments_page.instruments_card.click_tools_can_auto_gather_switch()
        admin_instruments_page.instruments_card.fill_tools_status_dropdown('Активный')
        admin_instruments_page.instruments_card.click_create_button()
    ...
    @staticmethod
    @pytest.mark.smoke
    @allure.title("Редактирование через контекстное меню записи в справочнике 'Инструменты'. Вкладка 'Постклик'")
    @allure.story(jira.JIRA_LINK + 'MDP-1166')
    @allure.testcase(case.ALLURE_LINK + '131398')
    def test_edit_tracker_context_menu(
            admin_base_url: str,
            admin_instruments_page: AdminOfficeInstrumentsPage,
            authorization_in_admin_office_with_token: str,
            clean_up_tracker_data,
    ):
        created_tracker_id = str(get_source_id_by_naming(data_instruments_card['naming']))
        admin_instruments_page.instruments.page.get_by_role('row', name=created_tracker_id).get_by_role(
            'button').click()
        ...
        data_instruments_card['url'] = 'https://vk.com'
        admin_instruments_page.instruments_card.fill_tools_name_field(data_instruments_card['name'])
        admin_instruments_page.instruments_card.fill_tools_url_field(data_instruments_card['url'])
        admin_instruments_page.instruments_card.click_tools_can_auto_gather_switch()
        admin_instruments_page.instruments_card.fill_tools_status_dropdown('Удален')
        ...
        admin_instruments_page.instruments.page.reload(wait_until='load')
        admin_instruments_page.instruments.check_instruments_table(data_instruments_card)
        delete_source_by_id(created_tracker_id)
