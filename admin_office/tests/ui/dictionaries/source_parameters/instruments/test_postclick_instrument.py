...
@pytest.fixture()
def clean_up_postclick_data(
        authorization_in_admin_office_with_token,
):
    """ Удаляет данные после выполнения всех тестов на вкладке 'Постклик' """
    postclick_id = get_source_id_by_naming(data_instruments_card['naming'])
    if postclick_id:
        delete_source_by_id(postclick_id)
...
class TestPostclick:
    @staticmethod
    @pytest.mark.smoke
    @allure.title("Создание записи в справочнике 'Инструменты'. Вкладка 'Постклик'")
    @allure.story(jira.JIRA_LINK + 'MDP-1166')
    @allure.testcase(case.ALLURE_LINK + '211165')
    def test_create_postclick(
            admin_base_url: str,
        ...
        admin_instruments_page.instruments_card.fill_tools_type_dropdown('Постклик')
    ...
    @staticmethod
    @pytest.mark.smoke
    @allure.title("Редактирование записи в справочнике 'Инструменты'. Вкладка 'Постклик'")
    @allure.story(jira.JIRA_LINK + 'MDP-1166')
    @allure.testcase(case.ALLURE_LINK + '131397')
    def test_edit_postclick(
            admin_base_url: str,
            admin_instruments_page: AdminOfficeInstrumentsPage,
            authorization_in_admin_office_with_token: str,
    ):
        created_postclick_id = str(get_source_id_by_naming(data_instruments_card['naming']))
        admin_instruments_page.instruments.page.get_by_role('row', name=created_postclick_id).get_by_role(
            'link').click()
    ...
    def test_edit_postclick_context_menu(
            admin_base_url: str,
            admin_instruments_page: AdminOfficeInstrumentsPage,
            authorization_in_admin_office_with_token: str,
            clean_up_postclick_data
    ):
        created_postclick_id = str(get_source_id_by_naming(data_instruments_card['naming']))
        admin_instruments_page.instruments.page.get_by_role('row', name=created_postclick_id).get_by_role(
            'button').click()
