...
class TestExportMediaPlanPlacementDone:
    """Тест создаёт рекламную кампанию и затем медиаплан, а также размещение. Размещение имеет статус "Выполнено".
    ...
    @allure.title(
        "Выгрузка медиаплана в xls формате, с размещением, которое имеет статус 'Выполнено'"
    )
    @allure.story('MDP-4475')
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.regress
    def test_export_mp_with_placement_done(
        ...
        campaign_id = CampaignsMutationsAPI(
            authorization_in_user_office_with_token).create_campaign_full(
            digital_test_data
        )
        ...
        placement_page.digital_placement_content.back_to_mediaplan_button.click()
        ...
        list_of_values_from_xls = find_names_by_coordinates(
            mediaplan_worksheet, values_coordinates_list
        )
        ...
        expect(sorted(list_of_values_from_api) == sorted(
            list_of_values_from_xls
        ), 'Data from Placement with Done status form and data from xls are not the same').to_be_true()
